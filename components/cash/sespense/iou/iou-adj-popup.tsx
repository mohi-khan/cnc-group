...
// ✅ localStorage থেকে last adjustment date পড়া
const getLastAdjDate = (): Date => {
  if (typeof window === 'undefined') return new Date()
  const saved = localStorage.getItem('iou_last_adj_date')
  if (saved) {
...
const IouAdjPopUp: React.FC<IouAdjPopUpProps> = ({
  ...
  const form = useForm<IouAdjustmentCreateType>({
    ...
    defaultValues: {
      iouId: iouId,
      amountAdjusted: 0,
      adjustmentDate: getLastAdjDate(), // ✅ last used date দিয়ে শুরু
      adjustmentType: 'refund',
      notes: '',
    },
  })

  // ✅ Dialog খোলার সময় last saved date set করো
  useEffect(() => {
    if (isOpen) {
      form.setValue('adjustmentDate', getLastAdjDate())
    }
  }, [isOpen, form])
  ...
  const onSubmit = async (data: IouAdjustmentCreateType) => {
    ...
    try {
      ...
      // ✅ Submit এর পর adjustment date localStorage এ save করো
      if (data.adjustmentDate) {
        localStorage.setItem(
          'iou_last_adj_date',
          format(new Date(data.adjustmentDate), 'yyyy-MM-dd')
        )
      }
      ...
      <DialogContent className="sm:max-w-[525px] max-h-[90vh] overflow-y-auto flex flex-col">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            {/* ✅ Adjustment Date - last used date দেখাবে */}
            <FormField
              name="adjustmentDate"
              control={form.control}
              render={({ field }) => (
