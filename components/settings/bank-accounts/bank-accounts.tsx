...
import { createBankAccount, editBankAccount } from "../../../api/bank-accounts-api"
...
import { getAllBankAccounts, getAllChartOfAccounts, getAllCompanies, getAllCurrency } from "@/api/common-shared-api"
...
export default function BankAccounts() {
  ...
  const fetchBankAccounts = React.useCallback(async () => {
    ...
    if (fetchedAccounts?.error?.status === 401) {
      ...
      toast({
        title: "Error",
        description: fetchedAccounts.error?.message || "Failed to get bank accounts",
      })
    } else {
  ...
  const fetchGlAccounts = React.useCallback(async () => {
    ...
    if (fetchedGlAccounts?.error?.status === 401) {
      ...
      toast({
        title: "Error",
        description: fetchedGlAccounts.error?.message || "Failed to get gl bank accounts",
      })
    } else {
    ...
    if (fetchedCompanies?.error?.status === 401) {
      ...
      setCompanies(
        (fetchedCompanies.data ?? [])
          .filter((c: any) => typeof c.companyId === "number" && typeof c.companyName === "string")
          .map((c: any) => ({
  ...
  const filteredAccounts = React.useMemo(() => {
    ...
    return accounts.filter((account) => {
      const companyName = companies.find((company) => company.companyId === account.companyId)?.companyName || ""

      const currencyCode = currency.find((curr) => curr.currencyId === Number(account.currencyId))?.currencyCode || ""
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <DialogTrigger asChild>
              <Button variant="default" className="bg-black hover:bg-black/90" onClick={() => form.reset()}>
                              ...
                              value={
                                field.value
                                  ? {
                                      ...
                                      name:
                                        BANGLADESH_BANKS.find((bank) => bank.name === field.value)?.name ||
                                        "Unnamed Bank",
                                    }
                                  : null
                              }
                              ...
                              value={
                                field.value
                                  ? {
                                      id: field.value.toString(),
                                      name:
                                        currency.find((curr: CurrencyType) => curr.currencyId === Number(field.value))
                        ...
                        render={({ field }) => (
                          <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4 w-full focus-within:ring-1 focus-within:ring-black focus-within:ring-offset-2 focus-within:rounded-md">
                            <div className="space-y-0.5">
                              ...
                              <FormDescription>Is this bank account active?</FormDescription>
                            </div>
                            <FormControl>
                        ...
                        render={({ field }) => (
                          <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4 w-full focus-within:ring-1 focus-within:ring-black focus-within:ring-offset-2 focus-within:rounded-md">
                            <div className="space-y-0.5">
                              ...
                              <FormDescription>Can this account be reconciled?</FormDescription>
                            </div>
                            <FormControl>
                              <Switch checked={field.value} onChange={field.onChange} />
                            </FormControl>
                              ...
                              value={
                                field.value
                                  ? {
                                      id: field.value.toString(),
                                      name:
                                        companies?.find((company) => company.companyId === field.value)?.companyName ||
                                        "Unnamed Company",
                                    }
                                  : null
                              }
          ...
          <TableBody>
            {paginatedAccounts.map((account) => (
              <TableRow key={account.id}>
                ...
                <TableCell>
                  {currency.find((curr) => curr.currencyId === Number(account.currencyId))?.currencyCode || "Unknown"}
                </TableCell>
                <TableCell>{account.accountType}</TableCell>
                <TableCell>{account.openingBalance}</TableCell>
                <TableCell>
                  {companies.find((company) => company.companyId === account.companyId)?.companyName}
                </TableCell>
