...
//                        }
//                        onChange={(item) => setCompanyId(item ? item.id.toString() : '')}
//                        items={company.map((comp) => ({
//                          id:
//                            comp.companyId !== undefined
//                              ? comp.companyId
//                              : 0,
//                          name: comp.companyName,
//                        }))}
...
import { toast } from '@/hooks/use-toast'
import { CustomCombobox } from '@/utils/custom-combobox'
import { createBudgetMaster } from '@/api/budget-api'
import { Checkbox } from '@/components/ui/checkbox'
import type { AccountsHead } from '@/utils/type'
import { CompanyType } from '@/api/company-api'
import { getAllChartOfAccounts } from '@/api/common-shared-api'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <CustomCombobox
                ...
                onChange={(item) =>
                  setCompanyId(item ? item.id.toString() : '')
                }
                items={company.map((comp) => ({
                  id: comp.companyId !== undefined ? comp.companyId : 0,
                  name: comp.companyName,
                }))}
                placeholder="Select Company"
              />
