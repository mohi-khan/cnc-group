...
// ─── Helper: response.error টা আসলেই error কিনা check করো ──────────────────────
// কখনো কখনো API {} empty object return করে, যেটা truthy কিন্তু আসলে error না
const isRealError = (error: unknown): boolean => {
...
export function JournalVoucherDetailsSection({
  ...
  // ─── fetchChartOfAccounts: isRealError দিয়ে {} empty object আলাদা করো ─────────
  const fetchChartOfAccounts = useCallback(async () => {
    if (!token) return
    try {
      const response = await getAllChartOfAccounts(token)

      // ✅ Fix: response.error যদি {} empty object হয় সেটা real error না
      if (isRealError(response.error) || !response.data) {
        if (isRealError(response.error)) {
          console.error('Error getting Chart Of accounts:', response.error)
          toast({
            title: 'Error',
            description:
              (response.error as any)?.message || 'Failed to get Chart Of accounts',
          })
        }
        return
      }
      ...
      console.error('Unexpected error fetching chart of accounts:', err)
      toast({
        title: 'Error',
        description: 'Failed to get Chart Of accounts',
      })
    }
  }, [token])

  // ─── fetchCostCenters: same fix ───────────────────────────────────────────────
  const fetchCostCenters = useCallback(async () => {
    if (!token) return
    try {
      const data = await getAllCostCenters(token)

      if (isRealError(data.error) || !data.data) {
        if (isRealError(data.error)) {
          console.error('Error getting cost centers:', data.error)
          toast({
            title: 'Error',
            description: (data.error as any)?.message || 'Failed to get cost centers',
          })
        }
        return
      }
      ...
      console.error('Unexpected error fetching cost centers:', err)
      toast({
        title: 'Error',
        description: 'Failed to get cost centers',
      })
    }
  }, [token])

  // ─── fetchDepartments: same fix ───────────────────────────────────────────────
    ...
    try {
      ...
      if (isRealError(response.error) || !response.data) {
        if (isRealError(response.error)) {
          ...
          toast({
            title: 'Error',
            description: (response.error as any)?.message || 'Failed to get departments',
          })
        }
        return
      }
  ...
  const fetchgetResPartner = useCallback(async () => {
    ...
    try {
      ...
      if (response?.error?.status === 401) {
        ...
        return
      } else if (isRealError(response.error) || !response.data) {
        if (isRealError(response.error)) {
          console.error('Error getting partners:', response.error)
          toast({
            title: 'Error',
            description: (response.error as any)?.message || 'Failed to load partners',
          })
        }
        setPartners([])
        return
  ...
  // ─── fetchgetCompanyChartOfAccounts: same fix ─────────────────────────────────
