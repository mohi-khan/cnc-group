...
const FdrValueUpdate = () => {
    ...
    const fetchFDRs = async () => {
      if (token) {
        try {
          ...
          console.log("fdr data: ", data.data? data.data:[])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // --- New Validation: New Value must be greater than old face value ---
    const invalidNewValues = rows.filter((row) => {
      const oldValue =
        fdrOptions.find((fdr) => fdr.fdrNo === row.fdrNo)?.faceValue ?? 0
      return row.newValue <= oldValue
    })

    if (invalidNewValues.length > 0) {
      toast({
        title: 'Validation Error',
        description: 'New value must be greater than the old face value',
        variant: 'destructive',
      })
      return
    }
              ...
              <div className="col-span-3">
                <Input
                  ...
                  value={
                    row.newValue !== 0
                      ? row.newValue
                      : (fdrOptions.find((fdr) => fdr.fdrNo === row.fdrNo)
                          ?.faceValue ?? '')
                  }
                  onChange={(e) =>
                    updateField(
