...
const [viewDetailVoucherId, setViewDetailVoucherId] = useState<number | null>(null)
  ...
  const currentTotal = useMemo(() => {
    return sortedVouchers.reduce(
      (acc, v) => {
        const details = (v.journalDetails as any[]) || []
        const topHeadRow = details.find(
          (d) => (d.accountsname || d.accountName) === v.topHead
        )
        const creditVal = topHeadRow?.credit ?? 0
        if (creditVal > 0) {
          acc.credit += creditVal
        } else {
          acc.debit += v.totalamount ?? 0
        }
        return acc
      },
      { debit: 0, credit: 0 }
  ...
  const draftCashVouchers = currentVouchers.filter(
    (v) =>
      v.state === 0 && v.journaltype === 'Cash Voucher' && canActOnVoucher(v)
  )
      ...
      <div className="max-h-[500px] overflow-y-auto">
        <Table className="border shadow-md">
          ...
          <TableBody>
                    ...
                    <TableRow key={`row-${voucher.voucherid}`}>
                        ...
                        <TableCell key={key} className="text-center text-xs">
                          {key === 'voucherno' ? (
                            ...
                            <span className="font-mono text-center block">
                              {(() => {
                                const details =
                                  (tooltipData[voucher.voucherid] as any[])
                                    ?.length > 0
                                    ? (tooltipData[voucher.voucherid] as any[])
                                    : (voucher.journalDetails as any[]) || []
                                const topHeadRow = details.find(
                                  (d: any) =>
                                ...
                                const creditVal = topHeadRow?.credit ?? 0
                                if (creditVal > 0) return '-'
                                return voucher.totalamount > 0
                                  ? `${voucher.currency ?? ''} ${formatIndianNumber(voucher.totalamount)}`
                                  : '-'
                              })()}
                            </span>
                          ) : key === 'credit' ? (
                      ...
                      <TableCell className="text-left">
                        <div className="flex gap-2 items-center justify-center">
                          {/* ✅ Eye icon - শুধু draft voucher এর জন্য */}
                          {voucher.state === 0 && (
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => {
                                setViewDetailVoucherId(voucher.voucherid)
                                fetchTooltipData(voucher.voucherid)
            ...
            {/* ✅ Total Row - main table এর ভেতরে শেষে */}
            {!isLoading && currentVouchers.length > 0 && (
                  ...
                  >
                    {key === 'voucherno' ? (
                      ...
                      <span className="font-mono">
                        {`Total Debit : ${formatIndianNumber(currentTotal.debit)}`}
                      </span>
                    ) : key === 'credit' ? (
                      <span className="font-mono">
                        {`Total Credit : ${formatIndianNumber(currentTotal.credit)}`}
                      </span>
                    ) : null}
                  </TableCell>
                ))}
...
{/* ✅ View Detail Dialog - Draft Voucher */}
<Dialog open={isViewDetailOpen} onOpenChange={setIsViewDetailOpen}>
  <DialogContent className="max-w-4xl">
    ...
    <div className="p-2">
      {viewDetailVoucherId && isLoadingTooltip[viewDetailVoucherId] ? (
        <div className="flex items-center justify-center py-12">
          <Loader />
        </div>
      ) : viewDetailVoucherId && tooltipData[viewDetailVoucherId] ? (
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse border">
            <thead>
              <tr className="bg-slate-100 border-b">
            ...
            <tbody>
              {(tooltipData[viewDetailVoucherId] as any[]).map((detail: any, index: number) => (
