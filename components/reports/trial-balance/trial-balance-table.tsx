...
const formatInternationalNumber = (num: number): string => {
  ...
  if (absNum < 1_000_000) return `${sign}${(absNum / 1_000).toFixed(1)}K`
  if (absNum < 1_000_000_000) return `${sign}${(absNum / 1_000_000).toFixed(2)}M`
  if (absNum < 1_000_000_000_000) return `${sign}${(absNum / 1_000_000_000).toFixed(2)}B`
  return `${sign}${(absNum / 1_000_000_000_000).toFixed(2)}T`
}

// ✅ Recursive function to calculate parent balances from children
const calculateParentBalances = (data: TrialBalanceData[]): TrialBalanceData[] => {
  return data.map((item) => {
    if (item.children && item.children.length > 0) {
      const updatedChildren = calculateParentBalances(item.children)
      const totals = updatedChildren.reduce(
      ...
      return { ...item, ...totals, children: updatedChildren }
    }
    return item
  })
}
...
export default function TrialBalanceTable({
  ...
  const fetchTrialBalanceTableData = useCallback(async () => {
    if (!token || !startDate || !endDate || !companyId) return

    try {
      // ✅ Correct local date (no UTC shift)
      ...
      console.log('From Date (local):', fromDate)
      console.log('To Date (local):', toDate)
      ...
      if (response.data) {
        const processedData = calculateParentBalances(response.data)
        setTrialBalanceDataLocal(processedData)
        setTrialBalanceData(processedData)
        console.log('Fetched trial balance data:', processedData)
      } else {
      ...
      <React.Fragment key={item.id}>
        <div
          ...
          className={`grid grid-cols-12 gap-4 ${
            !isGeneratingPdf ? 'cursor-pointer' : ''
          } p-2 border-b hover:bg-gray-100 ${
            expandedRows.has(item.id) ? 'font-bold bg-gray-50' : 'font-normal'
          } ${index % 2 === 0 ? 'bg-gray-50' : ''}`}
        >
          <div className="flex justify-center items-left">
            {!isGeneratingPdf && item.children?.length > 0 && (
              <span
                className={`text-xs ${
                  expandedRows.has(item.id) ? 'text-blue-600' : 'text-gray-600'
                }`}
              >
      ...
      <Card className="border rounded-lg">
        <CardContent>
          <div className="grid grid-cols-12 gap-4 p-2 pdf-table-header">
            ...
            <Card className="col-span-3 border">
              <CardHeader className="border-b p-2">
                <CardTitle className="text-center text-lg font-bold p-1">Period</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center">Debit</div>
            ...
            <Card className="col-span-3 border">
              <CardHeader className="border-b p-2">
                <CardTitle className="text-center text-lg font-bold p-1">Closing Balance</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center">Debit</div>
          ...
          <div>
            {!startDate || !companyId ? (
              <div className="text-center p-4 font-semibold animate-pulse">Please select a date range and company</div>
            ) : trialBalanceDataLocal.length > 0 ? (
              renderRows(trialBalanceDataLocal)
            ) : (
              <div className="text-center p-4">
