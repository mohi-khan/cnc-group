...
import { Card, CardContent } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Printer, RotateCcw, Check, Copy } from 'lucide-react' // Added Copy icon
import { toast } from '@/hooks/use-toast'
import {
  getSingleVoucher,
  reverseJournalVoucher,
...
export default function SingleGenralLedger() {
  ...
  const handleDuplicateVoucher = useCallback(() => {
    if (data && data[0]) {
      const voucherToDuplicate = data[0]
      // Navigate to a new page, passing relevant data for duplication
      // Assuming a route like /journal-voucher/create for creating new vouchers
      router.push(
        `/journal-voucher/create?duplicateVoucherId=${voucherToDuplicate.voucherid}&journalType=${voucherToDuplicate.journaltype}`
      )
    } else {
      toast({
        title: 'Error',
